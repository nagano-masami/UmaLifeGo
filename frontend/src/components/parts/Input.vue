...
export default {
  ...
  methods: {
    ...
    save: async function () {
      if (this.defaultBar.selectDate
        ...
        try {
          const result = await axios.post("https://umalifegobackend.onrender.com/saveRaceInfo", param);
          if (result.data === "OK") {
            // 保存に成功した場合
            alert("正常に保存できました");
            this.cancel();
  ...
  async mounted() {
    if (this.$store.state.bettingTicketId) {
      ...
      try {
        let result = await axios.post("https://umalifegobackend.onrender.com/getRaceInfo", param);
